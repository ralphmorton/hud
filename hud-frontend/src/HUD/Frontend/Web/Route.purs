...
instance pathableOAuthCode :: Pathable (OAuthCode a) where
    fromPathSegment = pure <<< OAuthCode <<< { unOAuthCode: _ }
    toPathSegment (OAuthCode c) = c.unOAuthCode

--
-- PathInfo
--

data PathInfo = PathInfo (Array String) (SM.StrMap String) (SM.StrMap String)
...
instance routeLit :: (Route p t a, IsSymbol s) => Route (L s :/ p) t a where
    route _ (PathInfo px qpx hpx) f = do
        let sval = reflectSymbol (SProxy :: SProxy s)
        r <- uncons px
        guard (r.head == sval)
        let pi = PathInfo r.tail qpx hpx
...
instance routeQueryParam :: (Route p t a, Pathable v, IsSymbol s) => Route (QueryParam s v :/ p) (Maybe v -> t) a where
    route _ pi@(PathInfo px qpx hpx) f = do
        let sval = reflectSymbol (SProxy :: SProxy s)
...
data HashParam (a :: Symbol) t

instance routeHashParam :: (Route p t a, Pathable v, IsSymbol s) => Route (HashParam s v :/ p) (Maybe v -> t) a where
    route _ pi@(PathInfo px qpx hpx) f = do
        let sval = reflectSymbol (SProxy :: SProxy s)
        case SM.lookup sval hpx of
            Nothing -> route (Proxy :: Proxy p) pi (f Nothing)
            Just sv -> do
                v <- fromPathSegment sv
                route (Proxy :: Proxy p) pi (f (Just v))

instance unrouteHashParam :: (Unroute p t, Pathable v, IsSymbol s) => Unroute (HashParam s v :/ p) (Maybe v :-> t) where
    unroute _ (Nothing :-> t) = unroute (Proxy :: Proxy p) t
    unroute _ ((Just v) :-> t) = base <> sep <> reflectSymbol (SProxy :: SProxy s) <> "=" <> toPathSegment v
        where
        base = unroute (Proxy :: Proxy p) t
